...
# Load in data 
```{r}
survey <- read.csv("C:/Users/lmlev/Documents/Github/BayerFellows/Survey_results_filtered_finished_MO.csv")
```
...
survey$know <- as.numeric((survey$kq13 + survey$kq15 + survey$kq17)/2)
survey$knowledge <- as.numeric((survey$kq13 + survey$kq15 + survey$kq17)/2)

survey$know[survey$know <= 1] <- "No previous knowledge"
survey$know[survey$know <= 1 & survey$know > ] <- "Some knowledge"
survey$know[survey$know <= 3 & survey$know > 2.25] <- "Previous knowledge"

```



# General Knowledge vs Actual Knowledge Ranking (q14)
```{r}
...
# FIGURE 1. Knowledge comparison among participants 
ggplot(know_sum, aes(Q14_1, count, fill = know)) + 
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Knowledge Rank by Participant", 
       fill = "Actual Knowledge Level", 
       y = "Responses") 
